...
public class PanelActDetails extends JPanel {
	 ...
	 * Create the panel.
	 */
	public PanelActDetails(ConnectedWindow frame, FacadeMain facade, Activity act) {
		this.facade = facade;
	...
	private void create() throws FalseFieldsException{
		...
		facade.getActFacade().createActivity(mapAct);
	...
	private void update() throws FalseFieldsException
	{
		...
		mapAct.put("name", textFieldName.getText());
		mapAct.put("actShortDescription", textFieldShortDesc.getText());
		mapAct.put("actDetailedDescription", textFieldDetailedDesc.getText());
		mapAct.put("pseudo", textFieldPseudo.getText());
		facade.getActFacade().updateActivity(mapAct);
		this.mainFrame.changePanel(new PanelActivities(this.mainFrame, this.facade));
	}
